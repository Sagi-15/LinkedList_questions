...
int length(Node *head){
    /*Base case => agar head=NULL matlab no node so return 0*/
    if(head==NULL){
        return 0;
    }
    /*Now length=1+length(of 2nd node onwards) ie 1->2->3->4->5 => 1+length(2->3->4->5)
        yeh agla part mil agar head=head->next ho jaaye.*/
