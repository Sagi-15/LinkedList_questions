...
void printReverse(Node *head){
    /*Recursion use karenge so 1->2->3->4->null let
        so, pehle divide 1 and 2->3->4->null 
        pehle small calculation can't kar as recursion se 
        1st element can't print so call recursion to
        print 2nd element onwards then print head*/
    /*Base case when no element return; baas*/
    if(head==NULL){
        return;
    }
    printReverse(head->next);//so recursion apna kaam karke print kar dega reverse order mei aage walle hame baas last mei need to print
    /*Small calculation*/
    cout<<head->data<<" ";
}